...
def test_get_plants_succulents(client, succulent):
    response = client.get(f"/plants_base/plants/{TypeChoice.SUCCULENT.value}/")
...
def test_get_plants_microgreen(client, microgreen):
    response = client.get(f"/plants_base/plants/{TypeChoice.MICROGREEN.value}/")
    assert response.status_code == 200
    assert response.json()[0] == {
...
def test_get_plants_flowers(client, flowers):
    response = client.get(f"/plants_base/plants/{TypeChoice.FLOWERS.value}/")
    assert response.status_code == 200
    assert response.json()[0] == {
        "color": "YELLOW",
